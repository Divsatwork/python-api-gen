...
class FlaskApiGenerator:
    ...
    def __createFile(self, filename, fileLocation):
        if filename is None:
    ...
    def __checkFile(self, filename, fileLocation):
    ...
    def __checkDir(self, dirLocation):
        ...
        if os.path.isdir(dirLocation):
            ...
            self.__makeDir(dirLocation)

    def __makeDir(self, dirLocation):
        if dirLocation is None:
    ...
    def createControllerFile(self, filename, fileLocation, apis=[]):
        if self.__checkFile(filename,fileLocation):
            # raise ValueError('Already existing controller exists')
            pass
        self.__checkDir(fileLocation)
        self.__createFile(filename, fileLocation) #File created
        if apis is None or apis == []:
            raise ValueError('No API provided to be written in the controller')
        self.__writeFileHeader(filename, fileLocation)
        for apiDetail in apis:
            if apiDetail['method'] == None:
                #Give error stating method not passed
                continue
            if apiDetail['method'].lower() == 'get':
                self.__writeGETapi(apiDetail, filename, fileLocation)
            elif apiDetail['method'].lower() == 'post':
                self.__writePOSTapi(apiDetail, filename, fileLocation)
        self.__writeFileFooter(filename, fileLocation)

    def __writeFileHeader(self, filename, fileLocation):
        with open(os.path.join(fileLocation,filename), 'w') as fp:
            fp.write(self.__FLASK_HEADER)

    def __writeFileFooter(self, filename, fileLocation):
        with open(os.path.join(fileLocation,filename), 'a') as fp:
            fp.write(self.__FLASK_FOOTER)

    def __writeGETapi(self, apiDetail, filename, fileLocation):
        ...
        with open(os.path.join(fileLocation,filename), 'a') as fp:
            ...
            if apiDetail['response_body'] == {} or apiDetail['response_body'] is None:
                ...
                self.__checkDir(os.path.join(fileLocation,'responses'))
                ...
                for response_param in apiDetail['response_body']['fields']:
                    if str == type(apiDetail['response_body']['fields'][response_param]):
                        ...
                        cf.write(FLASK_CLASS_INIT_SELF_STRING % response_param)
                cf.close()
                self.__createFile('__init__.py', os.path.join(fileLocation,'responses'))
    ...
    def __writePOSTapi(self, apiDetail, filename, fileLocation):
